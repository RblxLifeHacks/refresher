...
def refresh_cookie(auth_cookie):
    with open("log.txt", "a") as f:
        ...
        auth_ticket = req.headers.get(
            "rbx-authentication-ticket", "Failed to get authentication ticket")

        print(f"Authentication Ticket: {auth_ticket}")
        f.write(f"{datetime.now()} - Authentication Ticket: {auth_ticket}\n")
